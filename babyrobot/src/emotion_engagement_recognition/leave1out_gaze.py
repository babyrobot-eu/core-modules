...
def arg_parse():
    ...
    parser.add_argument("--seg_len", dest="seg_len", help="Segment length", default=1, type=int)
    parser.add_argument("--seg_overlap", dest="seg_overlap", help="Segment overlap", default=0, type=int)
    parser.add_argument("--seq_len", dest="seq_len", help="Number of segments per sequence", default=1, type=int)
...
if __name__ == '__main__':
    ...
    for train_dataset, valid_dataset, test_dataset in dataset_loader:
        ...
        model = MyNet(num_classes, 2, sequence_length, hidden_size, one_hot, cuda)
        if cuda:
            model = model.cuda()
