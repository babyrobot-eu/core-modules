...
COLOR_IMAGE_TOPIC = "/kinect4/sd/image_color_rect"  # Ros topic of the undistorted color image
DEPTH_MAP_TOPIC = "/kinect4/sd/image_depth_rect"  # Ros topic of the depth map warped into the color frame
CAMERA_INFO_TOPIC = '/kinect4/sd/camera_info'  # ROS topic containing camera calibration K
...
GPU_MEMORY = 0.85  # in [0.0, 1.0 - eps]; percentage of gpu memory that should be used; None for no limit
    ...
    for pid in range(coord3d_mat.shape[0]):
        ...
        for kid in range(coord3d_mat.shape[1]):
            br.sendTransform((coord3d_mat[pid, kid, 0], coord3d_mat[pid, kid, 1], coord3d_mat[pid, kid, 2]),
                 ...
                 camera_frame_id)
            p = Pose(position=Point(x=coord3d_mat[pid, kid, 0], y=coord3d_mat[pid, kid, 1], z=coord3d_mat[pid, kid, 2]))
...
if __name__ == '__main__':
    ...
    pub = rospy.Publisher('human_pose', PoseArray, queue_size=1)
