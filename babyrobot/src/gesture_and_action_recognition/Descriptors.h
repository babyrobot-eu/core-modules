...
void PrintDesc2Stream(std::ostringstream& ss, std::vector<float>& desc, DescInfo& descInfo, TrackInfo& trackInfo)
{
	...
	for(int i = 0; i < descInfo.ntCells; i++) {
		...
		for(int t = 0; t < tStride; t++)
			for(int j = 0; j < dim; j++)
				vec[j] += desc[pos++];
		for(int j = 0; j < dim; j++)
			ss << std::fixed << std::setprecision(7) << vec[j]*norm << "\t";
	}
}
void PrintDesc2File(FILE* fid, std::vector<float>& desc, DescInfo& descInfo, TrackInfo& trackInfo)
{
	int tStride = cvFloor(trackInfo.length/descInfo.ntCells);
	float norm = 1./float(tStride);
	int dim = descInfo.dim;
